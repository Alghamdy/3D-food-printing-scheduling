...
J = 4; % total number of jobs
% the lower bound for each job
p_LB = [13; 85; 180; 200]; 
% the upper bound for each job
p_UB = [14; 92; 188; 250]; %500; 1000];



% Hyper-parameter for the job utilisation constraint
% p_psi =  1; % 500 (this give same results as the paper);

P_ddline = [110; 200; 107; 210; 108; 220; 112; 110; 210; 28; 30; 118]; % deadline for each part

% the regularization parameter to control the weight of delay 
% minimization relative to PT minimization 
LAMBDA = [0, 5e-5, 1e-4, 1e-3, 1e-2, 1e-1, 0.5, 1, 10, 20];
% LAMBDA = 1;
...
b9 = zeros(I*J, 1);
%% minimum delay Eq. (Eq.7 in our formulatuion)
% this section won't be needed with the manual setting for the minimum delay for now
% Amd = [-diag(repmat(P_ddline, [J, 1])), zeros(I*J, 4*J)];
% for j = 1:J
%     Amd(I*(j-1)+1:I*j, (I*J)+ 2*J +j) = 1;
% end
% bmd = zeros(I*J, 1);
%% Delay time Eq. (Eq. 6 in the note)
Adt = zeros(J, I*J + 4*J);
for j = 1:J
...
ylabel('Total Delay Violation (hrs)')
xlabel('Beta Value')
